...
function makeState(
  overrides: Partial<VerdictPrivateState> = {}
): VerdictPrivateState {
  return {
...
function runVerify(
  state: VerdictPrivateState,
  rulesOverrides: Partial<typeof RULES> = {}
) {
...
describe("VERDICT 10-check circuit", () => {
  ...
  it("CLEAN move → verdict = clean (0), totalFlagged = 0", () => {
    ...
    console.log("CLEAN:", {
      verdict,
      totalChecks: ledger.totalChecks,
      totalFlagged: ledger.totalFlagged
    });
  ...
  it("CHECK 5: OUT OF BOUNDS → flagged (x = 1001 > 1000)", () => {
    const { verdict, ledger } = runVerify(
      makeState({ currPos: [1001n, 110n] })
    );
