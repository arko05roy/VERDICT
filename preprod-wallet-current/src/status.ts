...
const dustSnapshot = await readFile('./dust-snapshot.json', 'utf8').catch(() => undefined);
...
try {
  ...
  checkpointTimer = setInterval(() => {
    void wallet.dust.serializeState().then(async (snapshot) => {
      ...
      await writeFile('./dust-snapshot.json', snapshot, { mode: 0o600 });
  ...
  await writeFile('./dust-snapshot.json', synced.dust.serialize(), { mode: 0o600 });
  console.log('Saved current DUST state: ./dust-snapshot.json');
} finally {
